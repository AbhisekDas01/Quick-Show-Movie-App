...
// Inngest function to cancel booking and release the seats of show after 10 min if payment is not made
    ...
    async ({ event }) => {
        ...
        await sendEmail({
            ...
            body: bookingConfirmationTemplate({
                ...
                posterUrl: `${process.env.TMDB_IMAGE_BASE_URL}${booking?.show?.movie?.poster_path}`,
...
const sendNewShowNotifications = inngest.createFunction(
    ...
    async ({ event }) => {
        const { movieTitle, posterUrl, year, genre, description, bookingUrl } = event.data;
